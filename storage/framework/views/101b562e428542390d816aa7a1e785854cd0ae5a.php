        ...
        <div class="card">
            ...
            <div class="card-body">
                <dl class="row">
                    ...
                    <dd class="col-sm-9">
                        <?php echo e($submission->form->name ?? 'N/A'); ?>
                    ...
                    <dl class="row">
                        <?php $__currentLoopData = $mappedData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $label => $data): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php
                                /*
                                |--------------------------------------------------------------------------
                                | Normalize value FIRST (important)
                                |--------------------------------------------------------------------------
                                */
                                $baseValue = $data['value'] ?? '';

                                if (is_array($baseValue)) {
                                    $displayValue = implode(', ', array_filter($baseValue));
                                } else {
                                ...
                                /*
                                |--------------------------------------------------------------------------
                                | Append child dropdown value
                                |--------------------------------------------------------------------------
                                */
                                if (!empty($data['child_value'])) {
                                    $displayValue .= ' → ' . $data['child_value'];
                                }

                                /*
                                |--------------------------------------------------------------------------
                                | Append custom "Other" value
                                |--------------------------------------------------------------------------
                                */
                                if (($data['value'] ?? '') === 'Other'
                                    && !empty($data['child_custom_value'])) {
                                    $displayValue .= ' → ' . $data['child_custom_value'];
                                }
                            ?>
