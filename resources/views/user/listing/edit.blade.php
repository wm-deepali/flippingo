                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                           <form id="edit-submission-form" enctype="multipart/form-data"
                                    data-action="{{ route('listing.update', $submission->id) }}">
                                    @csrf
                                    @method('PUT')
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select name="country" class="form-control">
                                                ...
                                                @foreach($countries as $country)
                                                    <option value="{{ $country->id }}" @if($submission->country_id == $country->id)
                                                    selected @endif>
                                                        {{ $country->name }}
                                                    </option>
                                                @endforeach
                                    ...
                                    {{-- Hidden currency field (UI only) --}}
                                    <input type="hidden" id="price_currency" value="{{ $submission->currency ?? 'INR' }}">
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <select class="form-control parent-dropdown" name="{{ $fieldKey }}">
                                                            ...
                                                            @if(!empty($field['properties']['enableParentOther']) && $field['properties']['enableParentOther'] == true)
                                                                <option value="Other" @if($parentValue == 'Other') selected @endif>Other
                                                                </option>
                                                            @endif
                                                        </select>
                                                    ...
                                                    @foreach($field['properties']['checkboxes'] ?? [] as $checkbox)
                                                        ...
                                                        <div class="form-check">
                                                            <input class="form-check-input" type="checkbox"
                                                                name="{{ $field['properties']['field_id'] ?? '' }}[]" value="{{ $value }}"
                                                    ...
                                                    @foreach($field['properties']['radios'] ?? [] as $radio)
                                                        ...
                                                        <div class="form-check">
                                                            <input class="form-check-input" type="radio"
                                                                ...
                                                                value="{{ $optionValue }}" @if(($value ?? '') == $optionValue || $isSelected)
                                                                checked @endif>
                                                            <label class="form-check-label">{{ $label }}</label>
                                                        </div>
                                                    @endforeach
 ...
 <script>
        function updatePriceCurrencyEdit() {
            ...
            if ($countrySelect.length && $countrySelect.val()) {
                ...
                if (countryId !== {{ config('app.india_country_id', 229) }}) {
                    currency = 'USD';
                    symbol = '$';
                }
            }
